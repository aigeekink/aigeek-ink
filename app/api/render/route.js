...
// Composite tattoo centered on body photo at click position, scaled to mask diameter
...
async function compositeImages(bodyBase64, tattooBase64, cx, cy, radius, bodyW, bodyH) {
  return new Promise((resolve, reject) => {
    ...
    const tryComposite = () => {
      ...
      // Draw body photo as background
      ctx.drawImage(bodyImg, 0, 0, bodyW, bodyH)

      // Scale tattoo to fit within circle diameter (with slight padding)
      const diameter = radius * 2 * 0.95
      const tattooAspect = tattooImg.naturalWidth / tattooImg.naturalHeight
      let tattooW, tattooH
      if (tattooAspect >= 1) {
        tattooW = diameter
        tattooH = diameter / tattooAspect
      } else {
        tattooH = diameter
        tattooW = diameter * tattooAspect
      }

      // Draw tattoo centered at click position, with multiply blend for realism
...
export default function RenderPage() {
  ...
  const handleRender = async () => {
    ...
    try {
      // Composite tattoo onto body photo client-side
  ...
  const handleStartOver = () => {
    ...
    setClickPos(null); setResultUrl(null); setError(null)
    setTattooBase64(null); setTattooLoaded(false)
  }
    ...
    <main style={{ fontFamily: 'system-ui, -apple-system, sans-serif', maxWidth: '680px', margin: '0 auto', padding: '0 1rem' }}>
      ...
      {(step === STEPS.CLICK_TO_PLACE || step === STEPS.ADJUST_SIZE) && bodyPhotoUrl && (
        <>
          ...
          <div style={{ position: 'relative', borderRadius: '12px', overflow: 'hidden', border: '1px solid #e5e5e5', marginBottom: '1rem', cursor: 'crosshair' }} onClick={handlePhotoClick}>
            <img ref={photoImgRef} src={bodyPhotoUrl} alt="Your body photo" onLoad={handleImageLoad} style={{ width: '100%', display: 'block', userSelect: 'none' }} draggable={false} />
      ...
      {step === STEPS.RENDERING && (
        <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
          ...
          <p style={{ fontSize: '0.85rem', color: '#888', lineHeight: '1.6', maxWidth: '300px', margin: '0 auto' }}>The AI is making your design look freshly inked. This takes 15–30 seconds.</p>
