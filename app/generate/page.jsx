...
const ALL_PROMPT_SUGGESTIONS = [
  ...
  'Dark reaper holding an hourglass with ravens',
  'Occult geometry with an all-seeing eye',
  'Serpent coiled around a dagger with roses',
  'Memento mori skull crowned with wildflowers',
  'Fierce wolf mid-howl in a storm, geometric style',
  'Japanese oni demon with cherry blossoms and lightning',
  'Medusa with snakes, fine line, ethereal',
  'Anatomical heart wrapped in thorns',
  'Phoenix rising from black flames',
  'Koi fish transforming into a dragon',
  'Norse runes arranged in a Viking compass',
  'Moth drawn to a crescent moon, dotwork',
  'Samurai helmet with cherry blossoms, bold blackwork',
  'Sacred geometry flower of life with planets',
  'Scorpion with ornamental filigree, dark style',
  'Minimalist spine made of wildflowers',
  'Ouroboros serpent eating its own tail',
  'Clock melting into a raven, surreal style',
  'Lion face with crown, neo-traditional',
  'Dagger through a rose with smoke trails',
  'Microrealism eye with galaxy inside the iris',
  'Tribal sun with hidden wolf face',
  'Kraken tentacles wrapping around an anchor',
...
export default function GeneratePage() {
  ...
  const handleModelClick = (model) => {
    if (model.free) {
      ...
      setUpgradeReason(`${model.name} (powered by ${model.poweredBy}) unlocks with any paid pack.`)
  ...
  return (
    <main style={{ fontFamily: 'system-ui, -apple-system, sans-serif', maxWidth: '680px', margin: '0 auto', padding: '0 1.5rem' }}>
      ...
      {showUpgradeModal && (
        <div onClick={() => setShowUpgradeModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1.5rem' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: '16px', padding: '2rem', maxWidth: '380px', width: '100%', textAlign: 'center' }}>
            <div style={{ fontSize: '2rem', marginBottom: '0.75rem' }}>🔓</div>
            <h2 style={{ fontSize: '1.15rem', fontWeight: '700', color: '#111', marginBottom: '0.5rem' }}>Unlock this feature</h2>
            <p style={{ fontSize: '0.88rem', color: '#666', lineHeight: '1.7', marginBottom: '1.5rem' }}>{upgradeReason} Get access with any paid pack — plus HD designs, skin renders, and your Artist Handoff PDF.</p>
            <a href="/pricing" style={{ display: 'block', background: '#111', color: '#fff', padding: '12px', borderRadius: '10px', textDecoration: 'none', fontWeight: '600', fontSize: '0.95rem', marginBottom: '0.75rem' }}>See pricing — from $2.99</a>
            <button onClick={() => setShowUpgradeModal(false)} style={{ background: 'none', border: 'none', color: '#aaa', fontSize: '0.85rem', cursor: 'pointer' }}>Maybe later</button>
          </div>
        </div>
      )}
      ...
      <section style={{ padding: '2rem 0 1.5rem', textAlign: 'center' }}>
        <h1 style={{ fontSize: '1.8rem', fontWeight: '700', color: '#111', marginBottom: '0.5rem', letterSpacing: '-0.02em' }}>Design your next obsession.</h1>
      ...
      <section style={{ marginBottom: '1.75rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
          {MODELS.map((model) => (
            <div key={model.id} onClick={() => handleModelClick(model)} style={{ border: selectedModel === model.id ? '2px solid #111' : '1px solid #e5e5e5', borderRadius: '10px', padding: '0.875rem', cursor: 'pointer', position: 'relative', background: selectedModel === model.id ? '#fafafa' : '#fff', opacity: model.free ? 1 : 0.75, transition: 'border 0.15s' }}>
              {!model.free && <span style={{ position: 'absolute', top: '8px', right: '8px', fontSize: '0.85rem' }}>🔒</span>}
              {model.credits === 2 && <span style={{ position: 'absolute', top: '8px', left: '8px', background: '#f0f0f0', color: '#555', fontSize: '0.6rem', padding: '2px 6px', borderRadius: '999px' }}>2 credits</span>}
              <p style={{ fontWeight: '700', fontSize: '0.88rem', color: '#111', marginBottom: '1px', marginTop: model.credits === 2 ? '1rem' : '0' }}>{model.name}</p>
              <p style={{ fontSize: '0.7rem', color: '#888', marginBottom: '0.4rem', fontStyle: 'italic' }}>Powered by {model.poweredBy}</p>
      ...
      <section style={{ marginBottom: '1.75rem' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
          {currentModel.styles.map((style) => (
            <button key={style} onClick={() => setSelectedStyle(style)} style={{ fontSize: '0.82rem', color: selectedStyle === style ? '#fff' : '#444', background: selectedStyle === style ? '#111' : '#f5f5f5', border: selectedStyle === style ? '1px solid #111' : '1px solid #e5e5e5', borderRadius: '999px', padding: '5px 14px', cursor: 'pointer', transition: 'all 0.15s' }}>{style}</button>
          ))}
        </div>
      </section>
      ...
      <section style={{ marginBottom: '1.75rem' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '0.75rem' }}>
          {suggestions.map((s) => (
            <button key={s} onClick={() => setPrompt(s)} style={{ fontSize: '0.75rem', color: '#555', background: '#f5f5f5', border: '1px solid #e5e5e5', borderRadius: '999px', padding: '3px 10px', cursor: 'pointer' }}>{s}</button>
          ))}
        </div>
      </section>
      ...
      <section style={{ marginBottom: '1.75rem' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
          {PLACEMENTS.map((p) => (
            <button key={p.id} onClick={() => setSelectedPlacement(p.id)} style={{ fontSize: '0.82rem', color: selectedPlacement === p.id ? '#fff' : '#444', background: selectedPlacement === p.id ? '#111' : '#f5f5f5', border: selectedPlacement === p.id ? '1px solid #111' : '1px solid #e5e5e5', borderRadius: '999px', padding: '5px 14px', cursor: 'pointer', transition: 'all 0.15s' }}>{p.label}</button>
          ))}
        </div>
        {selectedPlacement === 'other' && (
          <input type="text" value={customPlacement} onChange={(e) => setCustomPlacement(e.target.value)} placeholder="e.g. behind the ear, ribcage, hand, finger..." style={{ marginTop: '0.75rem', width: '100%', padding: '10px 14px', fontSize: '0.9rem', border: '1px solid #ddd', borderRadius: '8px', outline: 'none', fontFamily: 'system-ui, -apple-system, sans-serif' }} />
        )}
      </section>
      ...
      <section style={{ marginBottom: '1.75rem' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {SIZES.map((s) => (
            <button key={s.id} onClick={() => setSelectedSize(s.id)} style={{ flex: 1, padding: '10px 0', textAlign: 'center', border: selectedSize === s.id ? '2px solid #111' : '1px solid #e5e5e5', borderRadius: '10px', cursor: 'pointer', background: selectedSize === s.id ? '#fafafa' : '#fff', transition: 'all 0.15s' }}>
      ...
      <section style={{ marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {COLOR_MODES.map((c) => (
            <button key={c.id} onClick={() => handleColorClick(c)} style={{ flex: 1, padding: '10px 0', textAlign: 'center', border: selectedColor === c.id ? '2px solid #111' : '1px solid #e5e5e5', borderRadius: '10px', cursor: 'pointer', background: selectedColor === c.id ? '#fafafa' : '#fff', opacity: c.free ? 1 : 0.7, position: 'relative', transition: 'all 0.15s' }}>
              {!c.free && <span style={{ position: 'absolute', top: '4px', right: '6px', fontSize: '0.65rem' }}>🔒</span>}
              <p style={{ fontSize: '0.82rem', fontWeight: '600', color: '#111', margin: 0 }}>{c.label}</p>
              {c.geekOnly && <p style={{ fontSize: '0.65rem', color: '#888', margin: '2px 0 0' }}>Geek Pack only</p>}
            </button>
          ))}
        </div>
      </section>
      ...
      {isGenerating && (
        <div style={{ marginBottom: '1.5rem', textAlign: 'center' }}>
          <div style={{ width: '100%', height: '4px', background: '#f0f0f0', borderRadius: '999px', overflow: 'hidden', marginBottom: '0.75rem' }}>
            <div style={{ height: '100%', width: `${loadingProgress}%`, background: '#111', borderRadius: '999px', transition: 'width 0.3s ease' }} />
          </div>
      ...
      {result && (
        <section style={{ marginBottom: '2rem' }}>
          ...
          <div style={{ position: 'relative', borderRadius: '12px', overflow: 'hidden', border: '1px solid #f0f0f0' }}>
            <img src={result.imageUrl} alt="AI generated tattoo design" style={{ width: '100%', display: 'block' }} />
          ...
          <div style={{ background: '#fafafa', border: '1px solid #f0f0f0', borderRadius: '12px', padding: '1.25rem', marginTop: '1rem', textAlign: 'center' }}>
            <p style={{ fontWeight: '600', color: '#111', marginBottom: '0.4rem', fontSize: '0.95rem' }}>This could be permanent.</p>
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap' }}>
              <a href="/pricing" style={{ display: 'inline-block', background: '#111', color: '#fff', padding: '10px 24px', borderRadius: '8px', textDecoration: 'none', fontSize: '0.9rem', fontWeight: '600' }}>Upgrade — from $2.99</a>
              <button onClick={() => { setResult(null); setPrompt('') }} style={{ background: '#f5f5f5', color: '#555', border: '1px solid #e5e5e5', padding: '10px 24px', borderRadius: '8px', fontSize: '0.9rem', cursor: 'pointer' }}>Try another</button>
            </div>
          </div>
        </section>
      )}
